			...
			<div className="hidden sm:block group my-2 max-w-5xl">
				<Link href={`/${type}/${slug}`}>
					<a className="font-bold sm:font-normal text-2xl sm:text-3xl lg:text-5xl text-left group-hover:underline font-akzidenz leading-tight sm:leading-none mb-2">
						{searchTerm === ''
							? title
							: title.split(new RegExp(_.startCase(searchTerm), 'g')).map((part, index) =>
									index ===
									title.split(new RegExp(_.startCase(searchTerm), 'g')).length - 1 ? (
										<span>{part}</span>
									) : (
										<>
											<span>{part}</span>
...
function highlight(text, term) {
	if (term === '') {
		...
		return text.replace(new RegExp(term, 'gi'), <span className="bg-yellow-200">{term}</span>)
	}
}
