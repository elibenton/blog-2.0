...
export default function PostPage({ source, frontmatter }) {
  ...
  return (
    <>
      <BlogSEO {...frontmatter} />
      <Nav title={title} date={format(parseISO(date), 'MMMM d, yyyy')} location={location} />
      <div className="flex flex-col sm:flex-row justify-between space-y-4">
        <div className="lg:ml-8 w-3/4">
          ...
          {description && <p className="text-lg max-w-3xl font-plex">{description}</p>}
        </div>
        <div className="self-start sm:self-center lg:mr-16 w-1/4">
          <ul className="border-l-2 border-black">
            {date && <li className="pl-4">{format(parseISO(frontmatter.date), 'MMMM d, yyyy')}</li>}
            {location && (
              <li className="pl-4">
                {location},&nbsp;
                {country && country}
          ...
          <div className="border-2 border-black h-auto p-4">
            ...
            <p className="text-gray-700 text-sm mb-4">
              View on <a href={`https://www.worldcat.org/isbn/${isbn}`}>World Cat</a>
            </p>
            <p className="text-gray-700 text-sm mb-4">
              Bookshop is an online bookstore that supports local, independent shops. A great
              alternative to Amazon, they gives 75% of their profits away. They also give 10% to me!
              (This is an affiliate link)
            </p>
            <a href={affiliateLink} target="_blank" rel="noreferrer">
              <button className="bg-black text-white p-2 rounded duration-200">Purchase</button>
            </a>
          </div>
        </div>
      </div>
...
async function callAPI(ISBN) {
  const res = await fetch(`https://api.bookish.tech/search?type=isbn&id=${ISBN}`)
