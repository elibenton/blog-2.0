...
export async function getFilePaths(layoutType) {
	...
	const filteredFrontmatter = allFrontmatter.filter(({ type }) => type === layoutType)
...
export async function getAllFilesFrontMatter() {
	return await Promise.all(
		fs.readdirSync(path.join(root, 'posts')).map(async (postSlug) => {
			const source = fs.readFileSync(path.join(path.join(root), 'posts', postSlug), 'utf8')
...
function groupByDate(post) {
	...
	if (postDate.isAfter(moment().subtract(2, 'year'))) {
		return {
			...post,
			display: postDate.format(`MMM YY`),
			tier: 'tier_1'
		}
	}
	// else if (postDate.isBetween(moment().subtract(3, 'year'), moment().subtract(1, 'year'))) {
	// 	if (['12', '1', '2'].includes(postDate.format('M'))) {
	// 		return {
	// 			...post,
	// 			display: `Winter ${postDate.format(`YY`)}`,
	// 			tier: 'tier_2'
	// 		}
	// 	} else if (['3', '4', '5'].includes(postDate.format('M'))) {
	// 		return {
	// 			...post,
	// 			display: `Spring ${postDate.format(`YY`)}`,
	// 			tier: 'tier_2'
	// 		}
	// 	} else if (['6', '7', '8'].includes(postDate.format('M'))) {
	// 		return {
	// 			...post,
	// 			display: `Summer ${postDate.format(`YY`)}`,
	// 			tier: 'tier_2'
	// 		}
	// 	} else {
	// 		return {
	// 			...post,
	// 			display: `Autumn ${postDate.format(`YY`)}`,
	// 			tier: 'tier_2'
	// 		}
	// 	}
	// }
